...
class UsersTable extends TableGateway
{
	...
	public function save($data)
	{
		...
		$row = $result->current(); 
		if ($row) {
			$this->update($this->getThisAsArray(), array($this->_cols['email'] => $data['email']));
	...
	/**
	 * Returns user based on email address
	 * @param string $email
	 * @return array $result
	 */
	public function getUserByEmailAddress($email)
	{
		return $this->select(array($this->_cols['email'] => $email))->toArray();
	}
}
